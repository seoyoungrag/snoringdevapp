...
public class FrequencyCalculator {
    ...
    public double getFreqSecondMax() {
        ...
        double maxAmpDB = 20 * Math.log10(0.125 / 32768);
        double maxAmpFreq = 0;
        double secondMaxAmpDB = 20 * Math.log10(0.125 / 32768);
        double secondMaxAmpFreq = 0;
        for (int i = 1; i < spectrumAmpOutDB.length; i++) {
            if (spectrumAmpOutDB[i] > maxAmpDB) {
            	secondMaxAmpDB = maxAmpDB;
            	secondMaxAmpFreq = maxAmpFreq;
                maxAmpDB = spectrumAmpOutDB[i];
                maxAmpFreq = i;
            } else
            if (spectrumAmpOutDB[i] > secondMaxAmpDB) {
            	secondMaxAmpDB = spectrumAmpOutDB[i];
            	secondMaxAmpFreq = i;
            }
        }
        int sampleRate = 44100;
        secondMaxAmpFreq =  secondMaxAmpFreq * sampleRate / fftLen;
        if (sampleRate / fftLen < secondMaxAmpFreq && secondMaxAmpFreq < sampleRate / 2 - sampleRate / fftLen
        		&& secondMaxAmpFreq>400 ) {
            int id = (int) (Math.round(secondMaxAmpFreq / sampleRate * fftLen));
            double x1 = spectrumAmpOutDB[id - 1];
            double x2 = spectrumAmpOutDB[id];
            double x3 = spectrumAmpOutDB[id + 1];
            double a = (x3 + x1) / 2 - x2;
            ...
            if (a < 0) {
                double xPeak = -b / (2 * a);
                if (Math.abs(xPeak) < 1) {
                	secondMaxAmpFreq += xPeak * sampleRate / fftLen;
                }
            }
        }
        return secondMaxAmpFreq;*/
        ...
        int sampleRate = 44100;
        for (int i = 1; i < spectrumAmpOutDB.length; i++) {
            if (spectrumAmpOutDB[i] > maxAmpDB && i * sampleRate / fftLen > 400 && i * sampleRate / fftLen <1500) {
                maxAmpDB = spectrumAmpOutDB[i];
                maxAmpFreq = i;
            }
        }
    ...
    public int getFreqSecondN() {
        ...
        int sampleRate = 44100;
        for (int i = 1; i < spectrumAmpOutDB.length; i++) {
            if (spectrumAmpOutDB[i] > maxAmpDB && i * sampleRate / fftLen > 400) {
                maxAmpDB = spectrumAmpOutDB[i];
                maxAmpFreq = i;
            }
        }
